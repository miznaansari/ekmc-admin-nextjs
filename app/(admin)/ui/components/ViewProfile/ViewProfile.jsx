...
const ProgressiveImage = ({ img, alt, sx = {} }) => {
  ...
  const highRes = img.gallery_cf_1080px_image_url || img.gallery_cf_720px_image_url || img.gallery_cf_480px_image_url || img.gallery_cf_360px_image_url;

  const [currentSrc, setCurrentSrc] = useState(lowRes || img.gallery_cf_original_image_url);
...
const formatTimeToAMPM = (timeStr) => {
  ...
  // Convert UTC time to current system local timezone
  const date = new Date();
      ...
      try {
        ...
        if (rolesRes?.data?.all_roles) {
          rolesRes.data.all_roles.forEach((r) => {
            if (r.id !== 2 && r.id !== 3 && r.id !== 4 && r.id !== "2" && r.id !== "3" && r.id !== "4") {
                      ...
                      <TableBody>
                        {paginatedEmployees.map((emp, index) => (
                          <TableRow key={emp.id}>
                            ...
                            <TableCell>
                              {rolesMap[emp.user_role_id] || `Role ${emp.user_role_id}`}
                            </TableCell>
                            <TableCell>
                          ...
                          {day?.is_holiday ? (
                            ...
                            Array.isArray(day?.slots) ? day.slots.map((slot) => (
                              <Typography
                                ...
                                variant="body2"
                              >
                                {formatTimeToAMPM(slot.opening_time)} – {formatTimeToAMPM(slot.closing_time)}
                              </Typography>
                            )) : "N/A"
                          )}
                        </TableCell>
            ...
            <DialogContent sx={{ p: 3 }}>
              <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", mb: 2, mt: 2 }}>
                ...
                <Typography variant="subtitle1" color="text.secondary" sx={{ mt: 1 }}>
                  {rolesMap[selectedEmployee.user_role_id] || `Role ${selectedEmployee.user_role_id}`}
                </Typography>
              </Box>
