  ...
  const fetchTemplates = async () => {
    try {
      ...
      const formatted = list.map((item) => ({
        ...
        created_at_formatted: formatUTCToLocal(item.created_at),
      }));
              ...
              <TableBody>
                {tableData.map((row, index) => (
                  <TableRow hover key={row.id}>
                    ...
                    <TableCell>
                      <Typography fontWeight={600}>{row?.id || "N/A"} - {row?.title || "N/A"}</Typography>
                    </TableCell>

                    <TableCell>{row.message}</TableCell>
                    <TableCell>{row?.subtitle || "N/A"}</TableCell>
                    <TableCell>{row.ttl}</TableCell>
                    <TableCell><FormattedDate value={row.created_at} /></TableCell>
