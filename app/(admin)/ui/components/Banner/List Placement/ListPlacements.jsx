...
const ListPlacements = () => {
  ...
  const fetchListPlacements = async () => {
    try {
      setLoading(true)
      ...
      console.log("placements- ", response)
  ...
  const handleOpenAddRoute = () => {
    ...
    setDrawerContent(<AddPlacement
      ...
      onSuccess={() => {
        ...
        setDrawerContent(null)
        fetchListPlacements();
        setAlert({ open: true, severity: "success", message: "Placement Added!!" })
      }}
    />)
  }

  //handle edit route drawer
  const handleOpenEditPlacement = (placement) => {
    setDrawerOpen(true)
    setDrawerContent(<EditPlacement
      placement={placement}
      onSuccess={() => {
        setDrawerOpen(false)
        setDrawerContent(null)
        fetchListPlacements();
        setAlert({ open: true, severity: "success", message: "Placement Updated!!" })
      }}
      onCancel={() => {
        setDrawerOpen(false)
        setDrawerContent(null)
  ...
  const handleStatusClick = async (placement) => {
    console.log(placement)
    const updatedStatus = placement.status === 1 ? 0 : 1;

    const payload = {
      title: placement.title,
      banner_placement: placement.banner_placement,
    ...
    try {
      ...
      if (response.status === 200) {
        fetchListPlacements();
        setAlert({ severity: "success", open: true, message: "Status Updated Successfully!!" })

      }

    } catch (e) {
      console.log("error during update status-", e);
      setAlert({ severity: "error", open: true, message: "Status Update failed!!" })
    }
  }
            ...
            <Table stickyHeader>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell sx={tableHeaderCellStyle}>PLACEMENT DESCRIPTION</TableCell>
                ...
                {placements?.length > 0 ? (
                  placements.map((placement, index) => (
                    ...
                    >
                      ...
                      <TableCell sx={{ ...tableCellStyle, fontSize: '12px' }}>
                        <FormattedDate value={placement.created_at} />
                      </TableCell>

                      <TableCell sx={{ ...tableCellStyle, fontSize: '12px' }}>
                        <FormattedDate value={placement.updated_at} />
                      </TableCell>


                      <TableCell sx={tableCellStyle}>
                        <Chip
                          onClick={() => handleStatusClick(placement)}
                          label={placement.status === 1 ? "Active" : "Inactive"}
                          sx={{
                            backgroundColor: placement.status === 1 ? "#e8f5e9" : "rgba(255,0,0,0.1)", // light green or light yellow
                            color: placement.status === 1 ? "#1b5e20" : "red", // dark green or orange
                            borderRadius: "999px",
                            px: 2,
                            py: 0.5,
                            fontWeight: 600,
