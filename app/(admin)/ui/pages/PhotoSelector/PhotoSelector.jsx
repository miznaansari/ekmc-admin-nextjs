...
const PhotoSelector = () => {
  ...
  const accept = async (item) => {
    try {
      ...
      await instance.put(`/api/crawler/v1/filter/image/${item.id}`, payload);
  ...
  return (
    <Box {...handlers} position="relative" sx={{ bgcolor: "#fff",
      ...
      <Box sx={{ display: "flex", mb: 2, zIndex: 99999, position: "absolute", top: 10, left: 10 }}>
        <img src={Logo.src || Logo} alt="Logo" style={{ maxWidth: "50px", marginRight: "15px" }} />
        <Box>
      ...
      <Stack direction="row" spacing={0} position="absolute" bottom={0} width={"100%"} justifyContent="center">
        <Button variant="contained" sx={{ borderRadius: 0, width: "50%",height:'50px' }} color="error"  onClick={() => handleSwipe("left")}>
          👎 Skip
        </Button>
        <Button variant="contained" sx={{ borderRadius: 0, width: "50%" }} color="success"  onClick={() => handleSwipe("right")}>
          👍 Approve
        </Button>
      </Stack>
